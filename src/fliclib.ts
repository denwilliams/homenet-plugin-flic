 ...
 *
 */
const FlicRawClient = function(inetAddress, port) {
	var enumValues = {
	...
	function onClose(had_error) {
		...
		setTimeout(function () {
			if (closing) return;
			socket.connect(socketArgs.port, socketArgs.host);
		}, 1000);
	}
 ...
 */
export const FlicConnectionChannel: any = (function() {
	var counter = 0;
 ...
 */
export const FlicScanner = (function() {
	var counter = 0;
 ...
 */
export function FlicClient(host, port) {
		...
		switch (opcode) {
			...
			case FlicEventOpcodes.ButtonSingleOrDoubleClickOrHold: {
				if (connectionChannels[event.connId]) {
					...
					if ((opcode == FlicCommandOpcodes.CreateConnectionChannel && event.error != "NoError") || opcode == FlicEventOpcodes.ConnectionChannelRemoved) {
